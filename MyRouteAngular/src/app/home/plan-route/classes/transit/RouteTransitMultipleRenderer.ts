...
declare global {
    namespace google.maps {
      interface Polyline {
        getBounds(startBounds:any);
      }
    }
  }
export class RouteTransitMultipleRenderer extends RoutePainter{
    polylines:google.maps.Polyline[][]=[];
    shadows:google.maps.Polyline[]=[];
    data=[];
    infowindow:google.maps.InfoWindow;
    public paintRoute(response: google.maps.DirectionsResult) {
        ...
        this._route.Map.fitBounds(bounds);
    ...
    private highlightRoute(index,position:google.maps.LatLng) {
        for(var j in  this.polylines ) {
            if(j==index) {
                //var color = '#0000ff';
                this.polylines[j].forEach(element => {
                    element.setMap(this._route.Map);
                });
                this.shadows[j].setMap(null);
                ...
                if(position!=null){
                    ...
                    this.infowindow.open(this._route.Map);
                }
            }
            else {  
                this.shadows[j].setMap(this._route.Map);
                this.polylines[j].forEach(element => {
                    element.setMap(null);
                });
            }
